...
class SocialBlueprint(Blueprint):
    ...
    def authorize(self, provider_name):
        """
        Starts OAuth authorization flow, will redirect to 3rd party site.
        """
        provider = self.get_provider(provider_name)
        callback_url = url_for(".callback", provider=provider_name, _external=True)
    ...
    def create_bp(cls, name, connection_adapter, providers, *args, **kwargs):
        ...
        bp.route('/login/<provider>')(login)
        bp.route('/callback/<provider>')(callback)
        return bp
...
def login(provider):
    return bp.authorize(provider)


def callback(provider):
    return bp.callback(provider)
